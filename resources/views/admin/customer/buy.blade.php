        ...
        <div class="admin-content-body">
            ...
            <div class="page-body">
                ...
                <form class="am-form am-form-horizontal" method="post"
                      action="{{ route('admin.customers.buy_update', $customer->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="am-form-group">
                        ...
                        <p style="margin-left: 100px;margin-top: -30px;color: #f4645f">*
                            注：每次只能添加一条数据,已有数据不可编辑，请删除后重新添加</p>
                    </div>

                    <div class="files">

                        @foreach($customer->course_properties as $item)
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-12 am-u-md-3">
                                    <input class="file1" type="text" name="c_hour[]" value="{{$item->c_hour}}" readonly>
                                </div>

                                <div class="am-u-sm-12 am-u-md-3">
                                    <input type="text" class="file1" name="c_price[]" value="{{$item->c_price}}"
                                           readonly>
                                </div>
        ...
        $(function () {
            $(".add_file").click(function () {
                ...
                html += '</select>' +
                    '</div>' +
                    '<div class="am-u-sm-12 am-u-md-3">' +
                    '<input placeholder="课程课时" class="file' + i + '" type="text" name="c_hour[]">' +
                    '</div>' +
                    '<div class="am-u-sm-12 am-u-md-3">' +
                    '<input placeholder="课时价格" type="text" class="file' + i + '" name="c_price[]">' +
                    '</div>' +
                    '<div class="am-u-sm-12 am-u-md-1">' +
            ...
            $(document).on('click', '.del_one', function () {
                ...
                if (confirm('删除后不可恢复，请慎重！')) {
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('admin.customers.del_one') }}",
                        data: {id: id},
                        success: function () {
